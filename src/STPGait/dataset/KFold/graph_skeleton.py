...
class GraphSkeletonKFoldOperator(SkeletonKFoldOperator[GraphSkeletonDataset, SkeletonKFoldConfig]):
    ...
    def set_sets(self, train_indices: np.ndarray, val_indices: np.ndarray, 
        ...
        sets[Separation.VAL] = GraphSkeletonDataset(self._x[val_indices], self._names[val_indices], 
            self._numeric_labels[val_indices], node_invalid=self._node_invalidity[val_indices])
        sets[Separation.TEST] = GraphSkeletonDataset(self._x[test_indices], self._names[test_indices], 
            self._numeric_labels[test_indices], node_invalid=self._node_invalidity[test_indices])
        sets[Separation.TRAIN] = GraphSkeletonDataset(self._x[train_indices], self._names[train_indices], 
            self._numeric_labels[train_indices], node_invalid=self._node_invalidity[train_indices])
