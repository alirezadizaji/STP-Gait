...
class Entrypoint(E):
    def __init__(self) -> None:
        ...
        self.org_model_res = "108_wifacct_gcn3l_balanced_s"
    ...
    def _calc_loss(self, x: OUT, data: IN) -> torch.Tensor:
        ...
        loss_sup = -torch.mean(o_main[torch.arange(y.numel()), y])
    ...
    def run(self):
        ...
        for self.current_K in range(self.kfold.K):
            with self.kfold:
                weight_save_dir = os.path.join("../results", self.org_model_res, "weights", f"{self.kfold.valK}-{self.kfold.testK}")

                files = os.listdir(weight_save_dir)
                if len(files) != 1:
                    print(f"@@@ WARNING: {weight_save_dir} directory should have exactly one saved file; got {len(files)} instead.", flush=True)
