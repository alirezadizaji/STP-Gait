    ...
    def __init__(self) -> None:
        ...
        kfold = GraphSkeletonKFoldOperator(
            config=SkeletonKFoldConfig(
                ...
                savename="processed_3c_0.5sup_"+ self.cond +".pkl",
                proc_conf=ProcessingGaitConfig(preprocessing_conf=PreprocessingConfig(critical_limit=120)
                , supervised_percentage=0.5, remove_labels= [Label.ANXIOUS, Label.SENSORY_ATAXIC, Label.PARETIC,
                Label.HYPOKENITC_FRONTAL, Label.ANTALGIC, Label.DYSKINETIC, 
                Label.FUNCTIONAL, Label.MOTOR_COGNITIVE, Label.SPASTIC]))
            )
        config = BaseConfig(
            try_num=95,
            try_name="cond3class_0.5sup_" + self.cond,
            device="cuda:0",
            eval_batch_size=32,
